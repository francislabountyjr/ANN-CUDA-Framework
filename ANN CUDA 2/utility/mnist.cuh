...
#define MNIST_CLASS 10
	...
	public:
		...
		// increases current step index and updates shared buffer if input parameter is true
		int next();

		// returns a pointer to input batch data
		Blob<float>* get_data() { return data_; }

		// returns a pointer to target batch data
