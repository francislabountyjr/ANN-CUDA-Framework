...
void MNIST::train(int batch_size, bool shuffle)
{
	if (batch_size < 1)
	{
		std::cout << "Batch size should be greater than 1.\n";
		return;
	}
...
void MNIST::test(int batch_size)
{
	if (batch_size < 1)
	{
		std::cout << "Batch size should be greater than or equal to 1.\n";
		return;
	}
...
void MNIST::get_batch()
{
	if (step_ < 0)
	{
		std::cout << "You must initialize the dataset first.\n";
		exit(-1);
	}
	...
	{
		std::copy(data_pool_[data_idx + i].data(), &data_pool_[data_idx + i].data()[data_size], &data_->ptr()[data_size * i]);
	}

	// copy taget with one-hot encoding
	for (int i = 0; i < batch_size_; i++)
	{
		std::copy(target_pool_[data_idx + i].data(), &target_pool_[data_idx + i].data()[MNIST_CLASS], &target_->ptr()[MNIST_CLASS * i]);
	}
}
...
void MNIST::load_data(std::string& image_file_path)
{
	...
	if (!file.is_open())
	{
		std::cout << "Download dataset from 'http://yann.lecun.com/exdb/mnist/'\n";
		exit(-1);
	}
	...
	std::cout << "Loaded " << data_pool_.size() << " items\n";
...
void MNIST::load_target(std::string& label_file_path)
{
	...
	// reads and one-hot encodes all labels and prepares the input buffer for labels
